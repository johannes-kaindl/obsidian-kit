...
|---|---|---|
| `obsidian-kit/pure` | `ThinkSplitter`, `parseSSE`, `normalizeEndpoint`, `resolveActiveEndpoint`, `clampInt`, i18n-Engine, `frontmatter` (seit v0.19.0), `capabilities` (seit v0.21.0), `pdf/*`, `KIT_VERSION`, sowie seit v0.27.0 `diff`, `callout`, `sha256`, `vault-path`, `filename-template`, `error_body`, `run-state`, `cooperative-yield`, `clipboard`, `cache-download`, `settings_schema` | **kein** obsidian-Import (Node-testbar, PROF-OBS-03/04) — per eslint erzwungen |
| `obsidian-kit/testing` | `createObsidianMock()` + alle Stubs (Obsidian-Test-Double) | nur im Test-Pfad, **nie** ins `main.js` gebündelt |
| `obsidian-kit/obsidian` | `collapsibleSection` (seit v0.12.0) · `ClockPort`/`realClock` (seit v0.14.0) · `confirmAction`/`ConfirmOptions` (seit v0.16.0, ab v0.16.1 UI-STANDARD-§2-konform) · `FolderSuggest` (seit v0.18.0) · `renderSettingDefinitions`/`settingBodyHost`/`refreshSettingsTab` (seit v0.25.0) · `buildEndpointList` (seit v0.26.0) · `copyToClipboard` + `buildHubInto`/`HUB_CSS` (seit v0.27.0) — obsidian/runtime-gekoppelte Helfer | darf obsidian importieren |
...
| `diff` (seit v0.27.0) | `diffLines(oldText, newText) → DiffLine[]` (LCS-Zeilendiff) · `groupHunks(diff) → Hunk[]` · `applySelection(diff, selected[]) → string`. ⚠️ Die Literale `"ctx"\|"add"\|"del"` sind **load-bearing**: Konsumenten bauen daraus CSS-Klassennamen — eine Umbenennung bricht das Styling still | — |
| `wrapCallout` (seit v0.27.0) | `wrapCallout(title, body, type, fold?: boolean\|"static") → string` — `> [!type]`-Kopf + zeilen-geprefixter Body; Leerzeilen werden zu `>` (Callout-Kontinuität). Der Kopf wird auf **eine** Zeile normalisiert: ein `\n` in Titel oder Typ zerriss den Callout sonst still | — |
| `sha256` (seit v0.27.0) | Klasse `Sha256` (`update(bytes)` · `digestHex() → string`) für chunkweises Hashen plus `sha256Hex(bytes)` / `sha256HexUtf8(text)`. 64-Bit-Längenzähler (Dateien > 4 GB), synchron und obsidian-frei — für Fälle, in denen `crypto.subtle` (async) sich durch jede aufrufende Funktion ziehen müsste | — |
| `vault-path` (seit v0.27.0) | `normalizeVaultDir(dir) → string` (Slash-Rauschen vorn und hinten weg) · `joinVaultPath(dir, file) → string` · `vaultDirname(path) → string` | — |
| `filename-template` (seit v0.27.0) | `buildFilename(template, subs, opts?) → string` · `sanitizeFilename(s, opts?) → string` mit `onInvalid: "strip" \| "replace"`. Unbekannte Platzhalter bleiben literal sichtbar, leeres/kaputtes Template fällt auf einen Fallback zurück. Die Substitutions-Map ist `Object.create(null)` + `Object.hasOwn` — ein Objekt-Literal ließ `{toString}` Funktionsquelltext in den Dateinamen schreiben | — |
| `error_body` (seit v0.27.0) | `errorMessageFromBody(body: unknown, opts?) → string \| null` · `errorMessageFromText(text, opts?)` — lesbare Meldung aus einem JSON-Fehlerkörper, Kaskade `error.message` → `error` → `message` → `detail` (die vierte Quelle deckt FastAPI/OpenWebUI und Django REST ab). Leerwerte fallen durch, Treffer werden getrimmt | — |
| `run-state` (seit v0.27.0) | `makeRunState(config) → RunStateOps` — Zustandsautomat für abbrechbare Langläufer, Phasen-Typ generisch. Zwei tragende Regeln: **ein Abbruch wird von einem Folgefehler nicht überschrieben** (Abbrechen reißt den Stream ab und erzeugt fast immer einen Folgefehler) und **eine Schreib-/Commit-Phase verweigert den Abbruch** (sonst verwaiste Datei bei gemeldeter Stornierung) | — |
| `cooperative-yield` (seit v0.27.0) | `createCooperativeYield(opts) → CooperativeYield` — Zwei-Schranken-Zeittor für CPU-lange Arbeit im Renderer: Fortschrittsmeldung und echter Makrotask-Break haben **getrennte** Zeitschranken. Ohne die Pause wird ein Abbrechen-Klick nie zugestellt und der Knopf ist Dekoration. Kern bleibt timer-frei, `now` wird injiziert (`NowPort`) | — |
| `clipboard` (seit v0.27.0) | `writeClipboard(text, opts?) → Promise<boolean>` — der `!clipboard`-Guard steht **vor** jedem Zugriff aufs `clipboard`-Property: in non-secure Contexts (ältere Android-WebViews, http) wirft schon das *Lesen* synchron, ein `try/catch` nur um `writeText()` fängt das strukturell nicht | — |
| `cache-download` (seit v0.27.0) | `streamIntoCache(opts) → Promise<StreamIntoCacheResult>` plus die Ports `CacheLike`/`FetchLike` — eine Datei per `body.tee()` in die Cache API streamen (ein Zweig schreibt, der andere zählt Fortschritt), speicherkonstant auch bei GB-großen Modellgewichten. Die Store-Klasse baut der Consumer selbst | — |
| `settings_schema` (seit v0.27.0) | `validateSettings(defaults, raw, schema) → T` plus Feldprüfer (`oneOf`, `clampIntField`, `clampFloatField`, `nonEmptyString`, `arrayOf`, `arrayThen`). **Ergänzung zu `mergeSettings`, kein Ersatz:** der Merge reicht jedes Feld aus `data.json` unbesehen weiter (gewollte Forward-Compat), der Schema-Prüfer baut eine geschlossene Welt aus geprüften Feldern | — |
| `copyToClipboard` (seit v0.27.0) | `copyToClipboard(text, opts?) → Promise<boolean>` — bindet `writeClipboard` an Obsidians `Notice`. Die Erfolgsmeldung erscheint **erst nach** erfolgreichem Schreiben; `failedMessage` hat den Default `"Copy failed"`, `null` schaltet stumm | — |
| `buildHubInto` (seit v0.27.0) | `buildHubInto(opts: HubOptions) → HubController` + Konstante `HUB_CSS` (Präfix `okit-hub-`) — Tab-Leiste, die **umbricht statt überzuläuft**, mount-once-Panels. ⚠️ `flex: 1` ist keine Kurzform von `flex: 1 1 auto`, sondern dessen Gegenteil (`1 1 0%`): ein `flex-wrap`-Container voller `flex: 1`-Items bricht **nie** um | **UI-STANDARD §4** |
