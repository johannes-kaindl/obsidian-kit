...
- **`clampInt`-Soll-Semantik** (kanonische Quelle: kuro `SettingsTab.clampInt`, 12 Call-Sites): Signatur `clampInt(value: string | number, min, max, fallback)`; `n = typeof value === "number" ? value : Number.parseInt(value, 10)` → bei `!Number.isFinite(n)` auf `fallback`, sonst `Math.max(min, Math.min(max, Math.trunc(n)))`. Verbindlich; die divergenten Inlinings (Number/parseFloat/`>0`-Guard) werden **darauf** vereinheitlicht. Floats → Int via `Math.trunc` (kein Round) — Call-Sites mit Float-Domäne (vault-rag `0..1`) prüfen, ob Int-Semantik passt; falls nicht, dort lokal lassen (nicht erzwingen).
